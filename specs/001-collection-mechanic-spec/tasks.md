...
- [X] T028 Verify button row layout in `lrcollectionmechanic.lrdevplugin/UI__MainDialog.lua` — Execute button on the left side of the action row, Close as the single standard action button on the right; no Dry Run, OK, or Cancel buttons present per research.md D-007 and ui-contract.md Button Row spec
